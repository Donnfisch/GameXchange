...
class Matches extends Component {
    ...
    return (
      <tbody>
        ...
        {matches.length > 0 &&
          nameFilter &&
          matches
            .filter(match => match.user.username === nameFilter)
  ...
  generateRequest = () => {
    ...
    const message = `Dear ${
      outboundTrade[0].user.username
    },%0A%0A  This is an autogenerated email from gameXchange. The gameXchange user "${username}" would like to offer the following trade:`;
    let outboundOffer =
      "%0A%0AGAMES YOU WILL BE SENDING THEM%0A---------------------------------------";
    for (let i = 0; i < outboundTrade.length; i++) {
      outboundOffer += `%0A${outboundTrade[i].game.title}(${
        outboundTrade[i].game.platform
      })`;
    }
    let inboundOffer =
      "%0A%0AGAMES YOU WILL BE RECEIVING%0A---------------------------------------";
    for (let i = 0; i < inboundTrade.length; i++) {
      inboundOffer += `%0A${inboundTrade[i].game.title}(${
        inboundTrade[i].game.platform
      })`;
    }
  ...
  renderRow = match => (
    <tr
      key={match.id}
      onClick={e => this.onClickGame(match.user.username, match.game.id, e)}
    >
          ...
          <React.Fragment>
            ...
            <div style={buttonDivOuter}>
              <div style={buttonDivInner}>
                ...
                <button type="button" onClick={this.generateRequest}>
                  {" "}
                  Generate Request{" "}
                </button>
              </div>
